...
func handleSend(w http.ResponseWriter, r *http.Request) {
	...
	// Fill in {{.OrderID}} and friends so records show the real order, not the
	// raw template text.
	subject = renderTemplate(subject, req.Data)
	body = renderTemplate(body, req.Data)

	metadata, _ := json.Marshal(req.Data)

	// Demo mode logs delivery instead of calling SES or SNS.
	status := "sent"
	sentAt := time.Now()
...
// renderTemplate substitutes {{.Key}} placeholders in a template with values
// from data. JSON numbers (float64) are formatted without trailing zeros so an
// order id renders as "42", not "42.00". Placeholders with no matching key are
// left untouched rather than becoming "<no value>".
func renderTemplate(text string, data map[string]interface{}) string {
	for k, v := range data {
		...
		text = strings.ReplaceAll(text, "{{."+k+"}}", s)
	}
	return text
}
